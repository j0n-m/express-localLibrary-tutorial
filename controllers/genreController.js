...
exports.genre_detail = asyncHandler(async (req, res, next) => {
  ...
  //error catch for invalid genre id parameter
  if (!mongoose.isValidObjectId(req.params.id)) {
    console.log('Invalid genre id')
    // No results.
  ...
  //error catch for potential genre ids but no data was returned
  if (genre === null) {
...
exports.genre_delete_post = asyncHandler(async (req, res, next) => {
  const [genre, allBooksByGenre] = await Promise.all([
    Genre.findById(req.params.id).exec(),
    Book.find({ genre: req.params.id }, "name summary").exec(),
  ]);
  // Double check that there are no books that has this genre (same as GET)
